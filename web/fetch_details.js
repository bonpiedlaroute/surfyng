...
function generate_details_page(data, surfyn = false) {
    ...
    if (surfyn == true) {
      var det_ad_div_mb = createNode("div");
      det_ad_div_mb.className = "mb-3";

      var det_ad_row_div = createNode("div");
      det_ad_row_div.className = "row";
      det_ad_div_mb.appendChild(det_ad_row_div);

      similar_container.appendChild(det_ad_div_mb);
    }
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (surfyn) {
        ad_link.className = "card col-md-6 py-2";
        ad_link.style = "text-decoration: inherit; color: inherit";

        det_ad_row_div.appendChild(ad_link);
        det_ad_div_mb.appendChild(det_ad_row_div);

        var det_ad_flex_div = createNode("div");
        det_ad_flex_div.className = "flex-column align-items-center justify-content-center";

        ad_link.appendChild(det_ad_flex_div);
      }


      var det_ad_image_container_div = createNode("div");
      // CREATE A DIV flex-column
      ...
      // ADD THE DIV flex-column TO ad_link
      // AND ADD det_ad_image_container_div TO THE DIV flex-column
      if (!surfyn)
        ad_link.appendChild(det_ad_image_container_div);
      else
        det_ad_flex_div.appendChild(det_ad_image_container_div);
      // CREATE THE CONTAINER DIV FOR det_ad_desc_containter
      // CREATE THE h4 AND ADD IT TO THE CONTAINER of det_ad_desc_container
      if (surfyn) {
        var det_ad_simple_div = createNode("div");
        var det_ad_simple_div_title = createNode("h4");
        det_ad_simple_div_title.className = "card-title py-2 mb-2 text-center";
        det_ad_simple_div_title.innerHTML = data[i].ANNOUNCE_TITLE;

        det_ad_simple_div.appendChild(det_ad_simple_div_title);
      }
      ...
      if (surfyn)
        det_ad_simple_div.appendChild(det_desc_container_div);
...

      {
        ...
        if (data[i].hasOwnProperty('CELLAR'))
          det_ad_cellar_span.innerHTML = data[i].CELLAR == "Non"? 0 : data[i].CELLAR;
      ...
      if (!surfyn)
        ...
        det_ad_flex_div.appendChild(det_ad_simple_div);
      ...
      if (!surfyn)
        ...
        det_ad_flex_div.appendChild(det_ad_notary_fees_frame_div);
      ...
      if (!surfyn)
        ...
        det_ad_flex_div.appendChild(det_ad_logo_container_div);
